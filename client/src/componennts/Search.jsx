...
const Search = ({ onSearch, onHoursResult, onPriceResult, onError }) => {
  ...
  } = useGetEquipmentsByCategory(selectedCategory);
    ...
    const fetchByPrice = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/equipments/byPrice/${priceValue[0]}/${priceValue[1]}`,
        );
  ...
  useEffect(() => {
    const fetchByHours = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/equipments/byHours/${sliderValue[0]}/${sliderValue[1]}`,
        );

        if (!response.ok) {
          onError("Error fetching hours data");
          return;
        }
        const data = await response.json();
        onHoursResult(data);
      } catch (error) {
        onError(error.message || "Some error occurred");
        console.log(error.message);
      }
    };
    ...
    <div>
      <div className="relative border-[1px] border-black flex items-center h-10">
        ...
        <form onSubmit={search}>
          <input
            ...
            name="search"
            id="search"
            className="w-full outline-none b placeholder:text-black placeholder:text-md placeholder:font-aeonik"
            placeholder="  Search Equiptment"
      ...
      <div id="dropdowns" className="mt-4 flex gap-8  lg:gap-[34px] flex-wrap">
        <select
          name="category"
          id="category"
          className="outline-none border-[1px] border-black py-2 px-6 w-[130px] md:w-[150px]"
          onChange={handleCategoryChange}
        >
          <option className="text-[#121212] font-aeonik text-sm lg:text-base">
            Category
          </option>
          {categories.map(({ alt, category }) => (
            <option
              key={category}
              className="text-[#121212] font-aeonik text-sm lg:text-base capitalize"
              value={alt}
            >
              {alt}
            </option>
        ...
        <details className="flex">
          <summary className="outline-none border-[1px] border-black py-2 px-6 w-[130px] md:w-[150px]">
            Hours
          </summary>
          <div>
            <Slider
              range
              className="text-eYellow"
              min={10}
              max={50000}
        ...
        >
          ...
          <option
            value="cat1"
            className="text-[#121212] font-aeonik text-sm lg:text-base"
          >
            Agco
          </option>
          <option
